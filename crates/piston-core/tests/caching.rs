use piston::{Device, DeviceRequest, OpTensor};
...
fn test_simple_caching() -> anyhow::Result<()> {
    ...
    let t1 = OpTensor::from_data(vec![1f32], 1, Device::CPU, false)
        .to(&device)?
        .square()?;
    ...
    let t2 = OpTensor::from_data(vec![2f32], 1, Device::CPU, false)
        .to(&device)?
        .square()?;
    ...
    let t3 = OpTensor::from_data(vec![3f32], 1, Device::CPU, false)
        .to(&device)?
        .square()?;
    ...
    let t4 = OpTensor::from_data(vec![4f32], 1, Device::CPU, false)
        .to(&device)?
        .square()?;
    ...
    let t5 = OpTensor::from_data(vec![5f32], 1, Device::CPU, false)
        .to(&device)?
        .square()?;
