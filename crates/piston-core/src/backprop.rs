            ...
            match node.op() {
                ...
                | LazyOp::Binary(Binary {
                    ...
                    let rhs_tracked = match &rhs {
                        TensorTypeOrScalarEnum::Tensor(t) => ctx.is_tracked(t),
                        TensorTypeOrScalarEnum::Scalar(_) => false,
                    };

                    // Only compute masks if at least one tensor is tracked
                    if lhs_tracked || rhs_tracked {
                        ...
                        // If both masks are 1 on the same point, we want to scale the
                ...
                LazyOp::WhereCond(WhereCond {
                    ...
                    // Only create zeros tensor if needed
                    if on_true_tracked || on_false_tracked {
                        ...
                        if let TensorTypeOrScalarEnum::Tensor(on_false) = on_false {
                            if ctx.is_tracked(&on_false) {
                                let f_grad = zeros.where_cond(condition.clone(), grad)?;
                ...
                LazyOp::Matmul(Matmul {
                    ...
                    // Only compute expensive gemm if the tensor is tracked
                    if ctx.is_tracked(&lhs) {
                ...
                LazyOp::Norm(NormOp::LayerNorm(Norm {
                    ...
                    // Compute gradients w.r.t scale (gamma)
                    if scale_tracked {
                    ...
                    // Compute gradients w.r.t bias (beta)
                    if bias_tracked {
