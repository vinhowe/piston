...
use crate::{
    Array, BindGroupLayoutDescriptor as BGLD, BindingMode, BuiltIn, DType, InvariantError, Kernel,
    KernelElement, KernelKey, KernelRenderable, KernelSource, Matmul, MatmulSpec, OpTensor,
    OperationError, Scalar, Stride, Vec2, Vec4, WgslFragment, WgslKernelBuilder, WgslPrimitive,
    WorkgroupCount, WorkgroupSize, Workload, gpu::dtype::WgslDType, rvec, wgc, wgs,
};
use glam::IVec3;
...
impl GEMM {
    ...
    fn write_getters<P: WgslPrimitive>(
        ...
        let W = P::W;
        let dtype = P::T::DT;
        builder.write_unpack(A.dtype());

        let a_getters = match A.dtype() {
            DType::F32 | DType::F16 => {
                wgsl! {
            ...
            DType::Q8_0F(_) | DType::Q8_0H(_) => {
                wgsl! {
                    fn getA(d0 : i32, d1 : i32, d2 : i32) -> vec4<'dtype> {
                        return unpack(A[getAIndexFromCoords3D(vec3<i32>(d0,d1,d2)) / 4]);
                    }

                    fn getAbsMax(d0 : i32, d1 : i32, d2 : i32) -> 'dtype {
        ...
        match A.dtype() {
            ...
            DType::Q8_0F(_) | DType::Q8_0H(_) => {
                builder.write_global(wgsl! {
                    fn getB(d0 : i32, d1 : i32, d2 : i32) -> 'dtype {
                        return 'dtype(B[getBIndexFromCoords3D(vec3<i32>(d0,d1,d2)) / 'W]);
                    }
                });
            }
            _ => return Err(InvariantError::UnsupportedDType(A.dtype()).into()),
    ...
    fn write_readers_and_writers<P: WgslPrimitive>(
        ...
        let aAccessor = match self.lhs.dtype() {
    ...
    fn render_scalar<P: WgslPrimitive>(
        ...
        let a_inner = match self.lhs.dtype() {
            DType::F32 | DType::F16 => {
                wgsl! {
        ...
        for row in 0..ROW_PER_THREAD {
            for col in 0..ROW_PER_THREAD {
                ...
                kernel_builder.write_main(wgsl! {
                    val = 'dtype(acc['row]['col]) + 'bias_val;
                    'writer
                });
            }
        }
    ...
    fn render_vectorized<P: WgslPrimitive>(
        ...
        let load_a_inner = match self.lhs.dtype() {
            DType::F32 | DType::F16 => {
