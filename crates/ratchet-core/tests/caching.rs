...
fn test_simple_caching() -> anyhow::Result<()> {
    ...
    let t1 = Tensor::from_data(vec![1f32], shape![1], Device::CPU)
        .to(&device)?
        .square()?;
    ...
    let t2 = Tensor::from_data(vec![2f32], shape![1], Device::CPU)
        .to(&device)?
        .square()?;
    ...
    let t3 = Tensor::from_data(vec![3f32], shape![1], Device::CPU)
        .to(&device)?
        .square()?;
    ...
    let t4 = Tensor::from_data(vec![4f32], shape![1], Device::CPU)
        .to(&device)?
        .square()?;
    ...
    let t5 = Tensor::from_data(vec![5f32], shape![1], Device::CPU)
        .to(&device)?
        .square()?;
