<script lang="ts">
	import type { ToyTokenizer } from '$lib/train/data/toy/types';
	...
	const runConfig = $derived(getLatestRun()?.config ?? null);
	...
	let tooltipTokenizer: PreTrainedTokenizer | ToyTokenizer | null = $state(null);
	...
	$effect(() => {
		if (tokenizer instanceof Promise) {
			...
			tooltipTokenizer = tokenizer ?? null;
		}
	});
	...
	> {
		...
		// Shift is 1 for autoregressive (decoder-only, encoder-decoder), 0 for encoder-only
		const shift = modelType === 'encoder' ? 0 : 1;
	...
	const chartOptions = $derived.by(() => {
		...
		const defaultPrefixLen = modelType === 'encoder' ? 0 : modelType === 'encoder-decoder' ? 1 : 1;
			...
			>
				{#await tokenizer then tokenizer}
					{#each visibleCompletions as completion, index (index)}
						...
						{@const modelType = runConfig?.model.topology ?? 'decoder'}
						{@const prefixLen =
							completionsData.decoderPromptLengths?.[index] ??
							(modelType === 'encoder' ? 0 : modelType === 'encoder-decoder' ? 1 : 1)}
						{@const showEncoderGrid =
							runConfig?.model.topology === 'encoder-decoder' && completionsData.encoderInputs}

						<div
							role="button"
							tabindex="0"
							class={`border cursor-pointer border-neutral-200 p-2 md:p-3 gap-x-2 ${runConfig?.model.topology === 'encoder-decoder' ? 'inline-grid grid-cols-[min-content_min-content_min-content_min-content]' : 'flex'} ${focus?.exampleIndex === index ? 'ring-2 ring-blue-300 ring-inset' : ''}`}
							onmouseenter={() =>
								(hoveredFocus = {
									exampleIndex: index,
						...
						>
							{#if showEncoderGrid}
								<div></div>
								<div
									class="text-2xs font-bold tracking-wider font-mono uppercase text-neutral-400 truncate"
								>
									Encoder
								</div>
								<div class="text-2xs font-bold tracking-wider font-mono uppercase text-neutral-400">
									→
								</div>
								<div
									class="text-2xs font-bold tracking-wider font-mono uppercase text-neutral-400 truncate"
							...
							{#if showEncoderGrid}
								<div class="font-mono text-sm flex items-start -space-x-px">
									{#each completionsData.encoderInputs?.[index] || [] as encId, encIdx (encIdx)}
										<span
											class={`relative ${hasTargets ? 'inline-grid grid-rows-2' : 'inline-flex items-center'} gap-[2px] leading-none ml-[2px] first:ml-0`}
										>
											<span class="bg-white text-neutral-800 p-0.5 border border-neutral-200"
												>{visualizeToken(decodeSingle(encId, tokenizer))}</span
											>
										</span>
									{/each}
								</div>
